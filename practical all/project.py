...
conn=sql.connect(host='localhost',user='root',passwd='password',database='employees')
cur = conn.cursor()
cur.execute('create table log_id(username varchar(25) primary key,passwrd varchar(25) not null )')
...
if n== 1:
     ...
     print()
     V_SQLInsert="INSERT  INTO log_id (username,password) values (" +  str (passwd) + ",' " + name + " ') "
...
print ("\t\t\t",time.ctime())
import mysql.connector as sql
conn=sql.connect(host='localhost',user='root',password='password',database='employees')
...
def register():
    ...
    v_sql_insert="insert into office values("+int(v_em_no)+",'" +v_em_name+"','"+v_em_dept+"',"+str(v_em_salary)+","+str(v_em_age)+")"
